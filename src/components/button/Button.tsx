...
const Button: FC<Props> = ({ ButtonName, onClick = () => {}, type, icon }) => {
  return (
    <button
      ...
      className="inline-flex justify-center lg:w-52 sm:w-28 items-center py-3 px-5 text-base font-semibold text-center rounded-3xl border bg-white border-gray-300 focus:ring-4 0 text-[#4A4DA6] hover:bg-sky-300 focus:ring-indigo-950"
    >
      <div>
        {icon && <img src={icon} />}
        {ButtonName}
